...
const TaskListView: React.FC<TaskListViewProps> = ({ tasks, onSelectTask, onCreateTask }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'in_progress': return 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20';
      case 'todo': return 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20';
      case 'backlog': return 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-500 border-zinc-200 dark:border-zinc-700';
    ...
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-[#0B101A] p-6 lg:p-10 overflow-hidden transition-colors duration-300">
      ...
      <div className="flex items-center gap-4 mb-8">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search directives, or IDs..."
            className="w-full bg-zinc-50 dark:bg-[#161B22] border border-zinc-200 dark:border-zinc-800 rounded-2xl py-4 pl-12 pr-4 text-zinc-900 dark:text-white placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all font-sans"
            value={searchTerm}
              ...
              filteredTasks.map(task => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400 truncate">{task.assigned_name || 'Unassigned'}</span>
                  </div>
