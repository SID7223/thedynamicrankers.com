        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className="flex justify-start group">
              <div className="flex max-w-[95%] lg:max-w-[85%] gap-2 lg:gap-3 flex-row items-start">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="relative group/content">
                    <div className={`px-3 py-2 lg:px-4 lg:py-2.5 rounded-2xl text-sm bg-zinc-100 dark:bg-[#11161D] text-zinc-900 dark:text-zinc-200 rounded-tl-none shadow-sm border border-transparent dark:border-zinc-800/50 ${isEditing ? 'ring-2 ring-indigo-500' : ''} break-words`}>
                      {msg.attachments && msg.attachments.length > 0 && (
                        <div className="space-y-2 lg:space-y-3 mb-2">
                          {msg.attachments.map((att, i) => (
                            <div key={i} className="rounded-xl overflow-hidden bg-black/5 dark:bg-black/20 p-1.5 lg:p-2 border border-black/5 dark:border-white/10">
                              {att.type.startsWith('image/') ? (
                                <img
                                  src={att.url}
                                  alt={att.name}
                                  className="max-w-full max-h-[250px] lg:max-h-[320px] object-cover rounded-lg cursor-pointer hover:opacity-90 transition-opacity"
                                ...
                                <a href={att.url} download={att.name} className="flex items-center gap-2 text-xs text-indigo-600 dark:text-indigo-200 hover:underline">
                                  ...
                                  <span className="truncate max-w-[120px] lg:max-w-[150px]">{att.name}</span>
                                </a>
                              )}
                            </div>
                          ))}
                      ...
                      {isEditing ? (
                        <div className="flex flex-col gap-2 min-w-[180px] lg:min-w-[200px]">
                          <textarea
                            autoFocus
        ...
        {attachments.length > 0 && (
          <div className="flex flex-wrap gap-2 lg:gap-3 mb-3 lg:mb-4 p-2 lg:p-3 bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800/50 shadow-sm">
            {attachments.map((att, i) => (
              <div key={i} className="relative group w-16 h-16 lg:w-20 lg:h-20 bg-zinc-100 dark:bg-zinc-800 rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700">
                {att.type.startsWith('image/') ? <img src={att.url} className="w-full h-full object-cover" alt="Attachment" /> : <div className="w-full h-full flex items-center justify-center text-zinc-400 dark:text-zinc-500"><FileText size={20} /></div>}
                <button onClick={() => setAttachments(prev => prev.filter((_, idx) => idx !== i))} className="absolute top-1 right-1 p-1 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X size={10} /></button>
              </div>
            ))}
            <button onClick={() => fileInputRef.current?.click()} className="w-16 h-16 lg:w-20 lg:h-20 rounded-xl border-2 border-dashed border-zinc-300 dark:border-zinc-800 flex items-center justify-center text-zinc-400 dark:text-zinc-600 hover:border-indigo-500/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all"><Plus size={20} /></button>
          </div>
        )}
