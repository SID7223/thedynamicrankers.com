...
const TaskDetailView: React.FC<TaskDetailViewProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    const s = statusWorkflow.find(sw => sw.value === status);
    const Icon = s ? s.icon : Circle;
    return <Icon size={14} />;
  };
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row h-full overflow-hidden bg-white dark:bg-[#0B101A] transition-colors duration-300">
      {/* Task Content */}
      <div className="flex-1 overflow-y-auto custom-scrollbar border-r border-zinc-100 dark:border-zinc-800/50">
        <div className="p-6 lg:p-10 border-b border-zinc-100 dark:border-zinc-800/50 bg-white/50 dark:bg-[#0B101A]/50 backdrop-blur-xl sticky top-0 z-10">
          <div className="flex items-center justify-between mb-8">
            <button onClick={onClose} className="p-2.5 bg-zinc-100 dark:bg-zinc-800/50 text-zinc-500 hover:text-zinc-900 dark:hover:text-white rounded-xl transition-all active:scale-95 border border-zinc-200 dark:border-zinc-700/50 flex items-center gap-2 pr-4">
              <ChevronLeft size={20} />
            ...
            <div className="flex items-center gap-3">
              {!isGlobal && (
                <>
                  {isEditing ? (
                    <button onClick={handleSave} className="p-2.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-xl hover:bg-emerald-500/20 transition-all"><Check size={18} /></button>
                  ) : (
          ...
          <div>
            <div className="flex items-center gap-3 mb-3">
              <span className="text-[10px] font-bold text-zinc-500 bg-zinc-100 dark:bg-zinc-800/50 px-2 py-1 rounded-lg border border-zinc-200 dark:border-zinc-700/50">{task.task_number}</span>
              {!isGlobal && (
        ...
        <div className="p-6 lg:p-10 space-y-12 pb-24">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex flex-col">
                      ...
                      {isEditing ? (
                        <select value={editBuffer.assigned_to || ''} onChange={(e) => setEditBuffer({...editBuffer, assigned_to: e.target.value || null})} className="bg-transparent text-zinc-900 dark:text-zinc-100 text-sm font-bold font-sans outline-none">
          ...
          {isGlobal && (
             <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">Global Command Center</h3>
                <p className="text-zinc-500 max-w-sm">This is the unified communication channel for all operatives. Broadcast strategic updates here.</p>
             </div>
          )}
        </div>
      </div>
